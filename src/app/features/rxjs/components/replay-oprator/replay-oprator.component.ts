...
@Component({
  selector: 'app-replay-oprator',
  templateUrl: './replay-oprator.component.html',
  styleUrls: ['./replay-oprator.component.scss'],
})
export class ReplayOpratorComponent {
  listData1: string[] = [
    'Angular',
    'Reactjs',
    'Javascript',
  ...
  subscribeUser1() {
    if (this.subscriptionMode1) {
      ...
      this.subscription1 = this.addVideosElementService.videoEmmit.subscribe(
        (response) => {
  ...
  subscribeUser2() {
    if (this.subscriptionMode2) {
      ...
      this.subscription2 = this.addVideosElementService.videoEmmit.subscribe(
        (response) => {
